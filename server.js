...
function pythOWOify(code) {
    return code
        ...
        .replace(/for\s+(\w+)\s+in\s+range\((\w+),\s*(\w+)\):/g, 'FOR $1 = $2 TO $3 THWEN')
        ...
        .replace(/# (.*)/g, (match, comment) => {
            ...
            const randomEmote = uwuEmotes[Math.floor(Math.random() * uwuEmotes.length)];
...
app.post('/chat', async (req, res) => {
    try {
        ...
        if (!isCodeRequest(message)) {
            res.write(`data: ${JSON.stringify({ content: "UwU~ I can onwy genewate code! Ask me to wite some code and I'll make it supew kawaii! (｡♥‿♥｡)" })}\n\n`);
            res.write('data: [DONE]\n\n');
            return res.end();
        }
